...
void get_got_base() {
	...
	close(fd);
	while((line = strtok_r(s, "\n\r", &saveptr)) != NULL) { s = NULL;
		if(strstr(line, "/chal") == NULL) continue;
...
int init(){
	...
	for(int i = 0; i < GOT_SIZE; i++){
		char new_func_name[10] = "code_";
		for(int j = 0; j < 1477; j++){
			if(got_index[i] == ndat[j]){
				int length = snprintf(NULL, 0, "%d", j);
				snprintf(&new_func_name[5], length + 1, "%d", j);
				break;
			}
		}
		//printf("func_name: %s\n", new_func_name);
		code = (int (*)(void)) dlsym(handle, new_func_name);
		error = dlerror();
		if(error != NULL) {
			fprintf(stderr, "%s\n", error);
			exit(EXIT_FAILURE);
		...
		void **got_table = (void**) (main_base + got_addr[i]);
		//printf("dl: %p\n", code);
		//printf("got: %p\n", *got_table);
		*got_table = code;
	}
